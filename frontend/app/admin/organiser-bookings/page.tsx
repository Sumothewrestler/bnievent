...
export default function OrganiserBookingsPage() {
  ...
  const fetchRegistrations = async (token: string) => {
    try {
      const response = await fetch('https://api.bnievent.rfidpro.in/api/special-registration/?category=ORGANISERS', {
      ...
      if (response.ok) {
        ...
        setError('Failed to fetch Organiser registrations')
      }
    } catch (err) {
  ...
  const handleGenerateIDCard = async (reg: SpecialRegistration) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `organiser_card_${reg.ticket_no}.png`
  ...
  if (!isReady || loading) {
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center', color: '#e74c3c', fontFamily: "'Inter', sans-serif" }}>
          <div style={{
            width: '50px', height: '50px',
            border: '4px solid #f3f3f3', borderTop: '4px solid #e74c3c',
            borderRadius: '50%', animation: 'spin 1s linear infinite',
            margin: '0 auto 20px',
          }}></div>
          <p style={{ fontSize: '16px', fontWeight: '600' }}>Loading Organiser Bookings...</p>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: 0, color: '#666', fontSize: '14px' }}>
              Total: {registrations.length} Organiser registrations
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div style={{
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'Inter', sans-serif" }}>
            <thead>
              <tr style={{ backgroundColor: '#fff5f5' }}>
                {['Ticket No', 'Organiser Name', 'Mobile', 'Company', 'Registered By', 'Date', 'QR Code', 'Actions'].map((h) => (
                  <th key={h} style={{
                    padding: '12px', textAlign: 'left', borderBottom: '2px solid #dee2e6',
                    fontSize: '14px', fontWeight: '600', color: '#e74c3c',
                  }}>{h}</th>
                ))}
              </tr>
            </thead>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '4px' }}>
                      <img
                        ...
                        style={{
                          ...
                          border: '2px solid #e74c3c', borderRadius: '4px',
                        }}
                      />
                      <a
                        ...
                        style={{ fontSize: '11px', color: '#e74c3c', textDecoration: 'underline' }}
          ...
          {filtered.length === 0 && (
            <p style={{
              ...
              {registrations.length === 0 ? 'No Organiser registrations yet' : 'No results found'}
            </p>
          )}
        </div>
      </div>
