...
export default function VolunteerBookingsPage() {
  ...
  const [registrations, setRegistrations] = useState<SpecialRegistration[]>([])
  ...
  const fetchRegistrations = async (token: string) => {
    try {
      const response = await fetch('https://api.bnievent.rfidpro.in/api/special-registration/?category=VOLUNTEERS', {
      ...
      if (response.ok) {
        ...
        setError('Failed to fetch Volunteer registrations')
      }
    } catch (err) {
  ...
  const handleGenerateIDCard = async (reg: SpecialRegistration) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `volunteer_card_${reg.ticket_no}.png`
  ...
  const handleViewIDCard = async (reg: SpecialRegistration) => {
    try {
      const token = localStorage.getItem('access_token')
      const response = await fetch(
        `https://api.bnievent.rfidpro.in/api/registrations/${reg.id}/generate-id-card/`,
  ...
  if (!isReady || loading) {
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center', color: '#f39c12', fontFamily: "'Inter', sans-serif" }}>
          <div style={{
            width: '50px', height: '50px',
            border: '4px solid #f3f3f3', borderTop: '4px solid #f39c12',
            borderRadius: '50%', animation: 'spin 1s linear infinite',
            margin: '0 auto 20px',
          }}></div>
          <p style={{ fontSize: '16px', fontWeight: '600' }}>Loading Volunteer Bookings...</p>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 20px' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: 0, color: '#666', fontSize: '14px' }}>
              Total: {registrations.length} Volunteer registrations
            </p>
          </div>
          <button
            onClick={() => {
              const token = localStorage.getItem('access_token')
              if (token) { setLoading(true); fetchRegistrations(token) }
            }}
            style={{
              padding: '10px 20px', backgroundColor: '#f39c12', color: '#fff',
        ...
        <div style={{
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'Inter', sans-serif" }}>
            <thead>
              <tr style={{ backgroundColor: '#fff8ee' }}>
                {['Ticket No', 'Volunteer Name', 'Mobile', 'Company', 'Registered By', 'Date', 'QR Code', 'Actions'].map((h) => (
                  <th key={h} style={{
                    padding: '12px', textAlign: 'left', borderBottom: '2px solid #dee2e6',
                    fontSize: '14px', fontWeight: '600', color: '#f39c12',
                  }}>{h}</th>
                ))}
              </tr>
            </thead>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '4px' }}>
                      <img
                        ...
                        style={{
                          ...
                          border: '2px solid #f39c12', borderRadius: '4px',
                        }}
                      />
                      <a
                        ...
                        style={{ fontSize: '11px', color: '#f39c12', textDecoration: 'underline' }}
          ...
          {filtered.length === 0 && (
            <p style={{
              ...
              {registrations.length === 0 ? 'No Volunteer registrations yet' : 'No results found'}
            </p>
          )}
        </div>
      </div>
