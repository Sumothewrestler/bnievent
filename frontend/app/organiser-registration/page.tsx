...
export default function OrganiserRegistrationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.registered_by) {
      setMessage('Please select who is registering this Organiser')
      setMessageType('error')
      return
    }
    ...
    try {
      const response = await fetch('https://api.bnievent.rfidpro.in/api/special-registration/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...formData, registration_for: 'ORGANISERS' }),
      })
      ...
      if (response.ok && data.success) {
        ...
        setMessage(`Organiser Registration successful! Ticket No: ${data.ticket_no}`)
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            color: '#e74c3c', fontWeight: '600', fontSize: '14px',
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: '35px' }}>
            ...
            <p style={{ color: '#666', margin: 0, fontSize: '14px' }}>
              Register as an Organiser for BNI CHETTINAD Event
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button
              ...
              style={{
                ...
                boxShadow: loading ? 'none' : '0 4px 15px rgba(231, 76, 60, 0.4)',
              }}
              onMouseOver={(e) => {
            ...
            >
              {loading ? 'Registering...' : '🧑‍💼 Register Organiser'}
            </button>
          </form>
        </div>
        ...
        <p style={{
          ...
          Organiser registrations are complimentary — no payment required.
        </p>
      </div>
    </div>
  )
